...
def publish_registration_event(registration_id: int, user_id: int, event_id: int):
    payload = {
    ...
    topic_name = "registration-created"
    print(f"Publishing registration event to topic '{topic_name}': {payload}")

    return send_message(topic=topic_name, data=payload)
